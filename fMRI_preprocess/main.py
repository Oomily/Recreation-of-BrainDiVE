...
ZSCORED_DIR = os.path.join(zscored_dir,'') # Directory containing the processed fMRIS (after z-score) will be stored

#SESS_NUM=40
SESS_NUM=1
STIM_FILE = "../nsd_stimuli.hdf5"
STIM_ORDER_FILE = "../nsd_expdesign.mat"
...
if __name__=='__main__':
    ...
    for sess in tqdm(range(SESS_NUM)):
        ...
        in_file = os.path.join(OUT_DIR, f'betas_session{sess+1:02}.hdf5')
        #out_file = os.path.join(ZSCORED_DIR, f'betas_session{sess+1:02}_s{SUBJ:02}_{RO}_zscored.hdf5')
        out_file = os.path.join(ZSCORED_DIR, f'betas_session{sess+1:02}_s_zscored.hdf5')
